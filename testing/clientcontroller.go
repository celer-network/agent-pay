...
func StartClientController(
	...
	args := append(
		...
		extraArgs...)
	// Basic preflight checks for better diagnostics
	if _, err := os.Stat(keystorePath); err != nil {
		return nil, fmt.Errorf("keystore not found: %s: %w", keystorePath, err)
	}
	if _, err := os.Stat(configPath); err != nil {
		return nil, fmt.Errorf("config not found: %s: %w", configPath, err)
	}
	os.MkdirAll(dataPath, os.ModePerm)
	binPath := outRootDir + "testclient"
	process := StartProcess(binPath, args...)

	// Retry-loop dial to tolerate slower startup; also fail fast if process exits
	...
	for {
		if time.Now().After(deadline) {
			process.Kill()
			return nil, fmt.Errorf("grpcDial localhost:%s failed: startup timeout", listenPort)
		}
		// Is process alive?
		// On Unix, signal 0 checks if the process is alive without sending a signal
		if perr := process.Signal(syscall.Signal(0)); perr != nil {
			return nil, fmt.Errorf("testclient exited before serving grpc on %s: %v", listenPort, perr)
		}
		// Attempt dial with a short per-attempt timeout
