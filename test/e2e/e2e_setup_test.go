...
func TestMain(m *testing.M) {
	...
	if *reuse == "" {
		...
		// Register the OSP (and optionally other routers) in the on-chain RouterRegistry
		if err := tf.RegisterRouters([]string{ospKeystore}); err != nil {
			chkErr(err, "registering routers")
		}
